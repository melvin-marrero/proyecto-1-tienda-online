...
export default function IphoneAdmin() {
   ...
   useEffect(() => {
      const fetchProductos = async () => {
         setLoading(true);
         try {
            const response = await fetch("https://bask-end-tiend-online.onrender.com/api/iphone");
            if (!response.ok) {
               throw new Error("Error al obtener productos");
            }
            const data = await response.json();
            setProductos(data); // Guardar todos los productos al inicio
         } catch (error) {
      ...
      fetchProductos();  // Llamar a la API para obtener todos los productos
   ...
   const handleEliminar = async (id) => {
      if (window.confirm("¿Estás seguro de que deseas eliminar este producto?")) {
         try {
            const response = await fetch(`https://bask-end-tiend-online.onrender.com/api/iphone/${id}`, {
               method: 'DELETE',
            });
   ...
   return (
      <div className="conten">
         ...
         <div className="containe-produc-panel">
            <ModalFormulario product={"iphone"} />
