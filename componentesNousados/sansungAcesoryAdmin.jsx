...
export default function SansungAcesoryAdmin(){
  ...
  useEffect(() => {
   ...
   const handleEliminar = async (id) => {
    if (window.confirm("¿Estás seguro de que deseas eliminar este producto?")) {
      try {
        const response = await fetch(`https://bask-end-tiend-online.onrender.com/api/sansungAcesory/${id}`, {
          method: 'DELETE',
        });
   ...
   if (error) return <div>Error: {error}</div>;
    return (
      <div className="conten">
        ...
        <ModalFormulario product={"sansungAcesory"}/>
